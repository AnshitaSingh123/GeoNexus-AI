...
export default function KnowledgeGraphPage({ setPage }) {
    // Dummy data for the graph visualization
    const nodes = [
        ...
        <PageWrapper title="Knowledge Graph" setPage={setPage}>
            ...
            <div className="flex items-center space-x-4 mb-6">
                <div className="relative flex-grow">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M8 4a4 4 0 100 8 4 4 0 000-8zM2 8a6 6 0 1110.89 3.476l4.817 4.817a1 1 0 01-1.414 1.414l-4.816-4.816A6 6 0 012 8z" clipRule="evenodd" /></svg>
            ...
            <div className="border-2 border-dashed border-gray-200 dark:border-gray-700 rounded-lg p-4 h-[50vh] flex items-center justify-center bg-gray-50 dark:bg-gray-800/30">
                    ...
                    <g>
                        {nodes.map(node => (
                            <g key={node.id} transform={`translate(${node.x}, ${node.y})`} className="cursor-pointer group">
                                <circle r="25" fill={node.color} className="transition-transform duration-200 ease-in-out group-hover:scale-110" />
                                <text textAnchor="middle" dy="5" fill="white" className="font-semibold text-xs select-none pointer-events-none">{node.id}</text>
                            </g>
                        ))}
                    </g>
                </svg>
